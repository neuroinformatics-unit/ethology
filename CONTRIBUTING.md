...
We recommended to use [conda]() to create a
...
Pre-commit hooks are a set of tools that run automatically before each commit and help maintain a consistent formatting style (see ###pre-commit).
### Pull requests

In all cases, please submit code to the main repository via a pull request (PR).
We follow our sister project [movement]() and adhere to the following conventions:
...
* Stage any fixes automatically made by the hooks, or fix any of them manually and commit.
* Make sure to add tests for any new features or bug fixes. See [testing]() below.
...
You may run the pre-commit hooks manually at any time on all staged files, by running `pre-commit run -a`.
