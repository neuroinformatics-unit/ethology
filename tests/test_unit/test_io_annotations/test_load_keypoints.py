...
class MockPoint:
    def __init__(self, x=None, y=None, visible=None, score=None, is_visible=None, confidence=None):
        self.x = x
        self.y = y
...
class MockFrame:
    def __init__(self, frame_idx=None, frame_index=None, frame_number=None, video=None, instances=None, user_instances=None, predicted_instances=None):
...
class MockLabels:
    def __init__(self, labeled_frames=None, frames=None, labeled_frames_by_video=None, skeletons=None, skeleton=None):
...
def test_get_instances():
    insts = [1, 2]
    assert load_keypoints._get_instances(MockFrame(user_instances=insts)) == insts
    assert load_keypoints._get_instances(MockFrame(instances=insts)) == insts
    assert load_keypoints._get_instances(MockFrame(predicted_instances=insts)) == insts
...
def test_points_from_instance():
    ...
    with pytest.raises(ValueError, match="Unsupported instance points format"):
        load_keypoints._points_from_instance(MockInstance(points_array=np.array([1])), 1)
...
def test_from_single_file(mock_require):
    ...
    frame = MockFrame(frame_idx=0, video=MockVideo("vid.mp4"), instances=[inst])
    sk = MockSkeleton(["kp1"])
...
def test_from_files_multiple(mock_single):
    ...
    ds1 = xr.Dataset(
        ...
        coords={"image_id": [0], "keypoint": ["kp1"], "id": [0], "space": ["x", "y"]}
    )
    ds1.attrs = {
