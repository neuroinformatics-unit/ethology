...
def sample_dataset(request, annotations_test_data: dict):
    ...
    fmt: Literal["VIA", "COCO"] = "VIA" if "VIA" in filename else "COCO"
...
class TestToNetcdf:
    ...
    def test_does_not_modify_caller_attrs(self, sample_dataset, tmp_path):
        ...
        assert isinstance(sample_dataset.attrs["map_category_to_str"], dict), (
    ...
    def test_dict_attrs_stored_as_json_strings_in_file(
        ...
        assert isinstance(raw_ds.attrs.get("map_category_to_str"), str), (
...
class TestFromNetcdf:
    ...
    def test_annotation_format_attr_preserved(self, saved_nc):
        nc_path, original = saved_nc
        loaded = from_netcdf(nc_path)
        assert loaded.attrs.get("annotation_format") == original.attrs.get(
            "annotation_format"
        )
    ...
    def test_all_data_variables_survive_round_trip(
        ...
        for var_name in sample_dataset.data_vars:
            np.testing.assert_array_equal(
                ...
                err_msg=(f"Round-trip mismatch in data variable '{var_name}'"),
            )

    def test_all_key_attrs_survive_round_trip(self, sample_dataset, tmp_path):
        ...
        for key in [
            ...
            assert reloaded.attrs.get(key) == sample_dataset.attrs.get(key), (
                f"Round-trip mismatch in attr '{key}'"
            )

    def test_reloaded_dataset_equals_original(self, sample_dataset, tmp_path):
