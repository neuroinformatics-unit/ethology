"""Module for Co-Tracker backend integration.

Provides functions to load and preprocess a video and run the Co-Tracker model
using custom query points.
"""
...
def preprocess_video(frames, device="cuda"):
    """Convert frames to a torch.Tensor of shape (B, T, C, H, W).

    on the specified device.
    """
    # Convert frames to a tensor, permute dimensions to
    # (T, C, H, W) and add batch dimension.
    video_tensor = (
...
def convert_query_points(input_coords, device="cuda"):
    """Convert input coordinates from [frame, y, x] to a tensor of.

        shape (num_points, 2) in [x, y] order.

    Args:
        input_coords (np.ndarray): Array of shape (num_points, 3)
            in [frame, y, x] format.
        device (str): Device to run the model on ("cuda" or "cpu").

    Returns:
        query_points (torch.Tensor): Tensor of shape
            (num_points, 2) in [x, y] order.
...
def run_co_tracker(
    video_path_or_url, input_coords, device="cuda", weights_path=None
):
    ...
    Args:
        ...
        input_coords (np.ndarray): Array of shape (num_points, 3)
            with [frame, y, x] coordinates.
        device (str): Device to run the model on ("cuda" or "cpu").
        weights_path (str, optional): Path to a checkpoint
