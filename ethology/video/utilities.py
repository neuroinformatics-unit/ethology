...
def compress_video(input_path: str, output_path: str, crf: int = 23, preset: str = "medium") -> None:
    """
    ...
    try:
        ...
        raise RuntimeError(f"Error compressing video: {e.stderr.decode()}") from e
...
def extract_frames(input_path: str,
                   ...
                   time_range: Optional[Tuple[Union[int, float], Union[int, float]]] = None,
    ...
    if all_frames:
        ...
        try:
            ...
            raise RuntimeError(f"Error extracting all frames: {e.stderr.decode()}") from e
        ...
        for frame in frame_numbers:
            ...
            try:
                ...
                raise RuntimeError(f"Error extracting frame {frame}: {e.stderr.decode()}") from e
        ...
        try:
            ...
            raise RuntimeError(f"Error extracting frames in time range: {e.stderr.decode()}") from e
    else:
        raise ValueError("Must specify either all_frames, frame_numbers, or time_range.")


def combine_images_to_video(image_pattern: str, output_video: str, fps: int = 30) -> None:
    """
    ...
    try:
        (
            ffmpeg
            .input(image_pattern, framerate=fps)
            .output(output_video, vcodec="libx264", pix_fmt="yuv420p")
            .run(quiet=True, overwrite_output=True)
        )
    except ffmpeg.Error as e:
        raise RuntimeError(f"Error combining images into video: {e.stderr.decode()}") from e


def crop_video_time(input_path: str, output_path: str,
    ...
    if start is None or end is None:
        raise ValueError("Must specify a valid time range either in seconds or in frame numbers.")
    ...
    try:
        ...
        raise RuntimeError(f"Error cropping video in time: {e.stderr.decode()}") from e


def crop_video_spatial(input_path: str, output_path: str,
    ...
    try:
        ...
        raise RuntimeError(f"Error cropping video spatially: {e.stderr.decode()}") from e